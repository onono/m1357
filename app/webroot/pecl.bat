...
FOR %%x IN ("C:\wamp\www\s0\app\webroot/PEAR") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=C:\wamp\www\s0\app\webroot/PEAR"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=C:\wamp\www\s0\app\webroot"
IF "%PHP_PEAR_PHP_BIN%"=="" SET "PHP_PEAR_PHP_BIN=c:\wamp\bin\php\php5.2.5\php.exe"
GOTO :INSTALLED
